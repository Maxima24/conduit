...
export function ScopeViewSwitcher({ value, onChange }: ScopeViewSwitcherProps) {
  ...
  return (
    <nav className="w-full md:w-auto" aria-label="SDK access views">
      <div className="relative grid w-full min-w-0 max-w-[460px] grid-cols-3 gap-0 overflow-hidden rounded-full bg-white/[0.04] p-1 md:min-w-[320px]" role="tablist">
        ...
        {VIEWS.map((view) => {
          ...
          return (
            <button
              ...
              className={`relative z-10 flex min-w-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-full border-0 bg-transparent px-2 py-2.5 font-sans text-[12px] font-semibold transition-colors duration-200 sm:gap-2 sm:px-3.5 sm:text-[13px] ${active ? 'text-[#f5f5f5]' : 'text-[#e2f0e7]/45 hover:text-[#f5faf7]/90'}`}
            >
